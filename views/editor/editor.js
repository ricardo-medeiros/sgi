(function( $ ){
	...
	var methods = {
		...
		imageWidget: function(){
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					...
					if(!f.type.match('image.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ //Process only Images
						methods.showMessage.apply(this,["imgErrMsg","Tipo de Arquivo Invalido"]);
						continue;
					}
			...
			var getURL = $("<button/>",{
				...
				if(url ==''){
					methods.showMessage.apply(this,["imgErrMsg","Por favor insira URL da imagem"]);
					return false;
				}
				...
				var image = $('<img/>',{
					src:url,
				  	...
				  	methods.showMessage.apply(this,["imgErrMsg","URL da imagem Invalida"]); 
				  	return false;
					...
					id:"tblForm"+idExtn 
						...
						class:"row" 
							...
							class:"col-xs-12 col-sm-6 col-md-6 col-lg-6"
								...
								).append($('<option/>',{ text:"Escolher", value:""}
								)).append($('<option/>',{ text:"Esquerdo", value:"left"}
								)).append($('<option/>',{ text:"Centralizado", value:"center"}
								)).append($('<option/>',{ text:"Direito",	value:"right"}))
								...
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellspacing"+idExtn,	text:"Espacamento da Celula"}
							)).append($('<input/>',{
								id:"tblCellspacing"+idExtn,
								type:"text", 
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellpadding"+idExtn,	text:"Preenchimento da Celula"}
							)).append($('<input/>',{
		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
							 ...
							 'font_size': {	"select":true,
							  ...
							  'color'	: { "text":"A",
											...
											"custom":function(button){
													...
													var palette       = $('<ul />',{id:"color_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).html('<div>Cor do textor</div>'));
													var bgPalletteDiv = $('<div/>',{id:"bg_colorpellete"});
													var bgPallette    = $('<ul />',{id:"bgcolor_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).html('<div>Cor de Fundo</div>'));
						     ...
						     'ul'		: { "text": "Bullet", 
											"icon": "fa fa-list-ul", 
											"tooltip": "Inserir / Remover lista com marcadores", 
						   ...
						   'insert_link': { "modal": true,
											...
											"onSave":function(){
												...
												if(targetURL ==''){
													methods.showMessage.apply(editorObj,["errMsg","Por favor entre com url"]);
													return false;
												}												
												if(!targetURL.match(urlPattern)){
													methods.showMessage.apply(editorObj,["errMsg","Url invalida"]);
													return false;
												}													
												if(range=='' && targetText==''){ 
						   ...
						   'insert_img'	: { "modal": true,
											...
											"onSave": function(){
												...
												if($('#imageList').data('current')){
													...
													methods.showMessage.apply(this,["imgErrMsg","Por favor selecione a imagem"]);
													return false;
												}
						...
						'insert_table'	: { "modal": true,
											...
											"onSave": function(){
												...
												if(!tblRows.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","Linhas deve ser um numero positivo"]);
													return false;
												}													
												if(!tblColumns.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","Colunas deve ser um numero positivo"]);
													return false;
												}
												if(tblWidth!="" && !tblWidth.match(cssReg)){
													methods.showMessage.apply(this,["tblErrMsg","Por favor, indique numero positivo com ou sem uma unidade de medida CSS valido ( px, em, ex, % , in, cm , mm , pt, pc)"]);
													return false;
												}
												...
												if(tblBorder!="" && !tblBorder.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","tamanho da borda deve ser um número positivo"]);
													return false;
												}
												if(tblCellspacing!="" && !tblCellspacing.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","espacamento de celula deve ser um numero positivo"]);
													return false;
												}
												if(tblCellpadding!="" && !tblCellpadding.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","preenchimento de celula deve ser um numero positivo"]);
													return false;
												}
						   ...
						   'indent'		: { "text": "Aumentar", 
											"icon":"fa fa-indent", 
											"tooltip": "aumentar avanco", 
											"commandname":"indent", 
											"custom":null },
						   ...
						   'outdent'	: { "text": "Recuar", 
							...
							'print'		: { "text": "Imprimir", 
											...
											var oPrntWin = window.open("","_blank","width=800,height=600,left=400,top=100,menubar=yes,toolbar=no,location=no,scrollbars=yes");
											oPrntWin.document.open();
											oPrntWin.document.write("<!doctype html><html><head><title>Imprimir</title></head><body onload=\"print();\">" + oDoc.html() + "</body></html>");
											oPrntWin.document.close();
											}},

							'rm_format'	: { "text": "Remover formato", 
											"icon":"fa fa-eraser", 
							...
							'togglescreen':{ "text": "tela de alternancia",
											 "icon": "fa fa-arrows-alt",
											 "tooltip": "tela de alternancia",
											 "commandname":null,
							...
							'source'	: { "text": "Codigo Fonte", 
											"icon":"fa fa-code", 
											"tooltip": "Codigo Fonte", 
											"commandname":null, 
		...
		createTableContext: function(event,cMenuUl){
			...
			var modalId="editProperties";
       		var modalHeader="Propriedades da Tabela";
       		...
       		var onSave = function(){ 
				...
				if(tblWidthEdt!="" && !tblWidthEdt.match(tblEdtCssReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","Por favor, indique numero positivo com ou sem uma unidade de medida CSS valido ( px, em, ex, % , in, cm , mm , pt, pc)"]);
					return false;
				}
				if(tblHeightEdt!="" && !tblHeightEdt.match(tblEdtCssReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","Por favor, indique número positivo com ou sem uma unidade de medida CSS válido ( px, em, ex, % , in, cm , mm , pt, pc)"]);
					return false;
				}
				if(tblBorderEdt!="" && !tblBorderEdt.match(tblEdtNumReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","tamanho da borda deve ser um numero positivo"]);
					return false;
				}
				if(tblCellspacingEdt!="" && !tblCellspacingEdt.match(tblEdtNumReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","espacamento de celula deve ser um numero positivo"]);
					return false;
				}
				if(tblCellpaddingEdt!="" && !tblCellpaddingEdt.match(tblEdtNumReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","preenchimento de celula deve ser um numero positivo"]);
					return false;
				}
			...
			cMenuUl.append($('<li/>',{class:"dropdown-submenu",css:{display:"block"}})
       						.append($('<a/>',{"tabindex":"-1", href:"javascript:void(0)","text":"Linha"}))
       						.append($('<ul/>',{class:"dropdown-menu"})
       								...
       								.append($('<li/>').append($('<a/>',{text:"Remover Linha"}).click( 
											function(e) { return function(){ 
													$('#context-menu').remove();
													$(e.target).closest("tr").remove(); 
											}}(event))))
       			)).append($('<li/>',{class:"dropdown-submenu",css:{display:"block"}})
   						.append($('<a/>',{"tabindex":"-1", href:"javascript:void(0)","text":"Coluna"}))
   						.append($('<ul/>',{class:"dropdown-menu"})
   								.append($('<li/>').append($('<a/>',{
										id:"tbl_addcolumn",
										"href":"javascript:void(0)",
   								...
   								.append($('<li/>').append($('<a/>',{text:"Remover Coluna"}).click( 
										function(e) { return function(){ 
												$('#context-menu').remove();
												var selectedCell = $(e.target);
												var columnIndex = selectedCell.siblings().addBack().index(selectedCell);
			...
			cMenuUl.append($('<li/>').append(modalTrigger))
					...
					.append($('<li/>').append($('<a/>',{text:"Remover Tabela"}).click( 
						function(e){ return function(){ 
								$('#context-menu').remove();
								            	...
								            	class:"modal-dialog"
							            			...
							            			class:"modal-content"
									            		...
									            		class:"modal-footer"
								                			...
								                			type:"button",
								                			class:"btn btn-success",
								               				}).html('Salvar').mousedown(function(e){
								                			e.preventDefault();
	...
	$.fn.Editor = function( method ){
		...
		if ( methods[method] ) {
			...
			$.error( 'Metodo ' +  method + ' nao existe no jQuery.Editor' );
		}    
	}; 
})( jQuery );
